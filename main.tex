...
\paragraph{Per-neuron embeddings (H3).} H3 asks whether the encoder represents each neuron
consistently, which requires one embedding vector per neuron per frame. Because the encoder preserves
spatial resolution, its output for a frame is a dense feature map that assigns a $32$-dimensional
feature vector to every location. For a given neuron I take its footprint from the label mask and
average the feature vectors of all locations inside that footprint, giving a single $32$-dimensional
embedding for that neuron in that frame (masked average pooling, Figure~\ref{fig:h3emb}). Doing this
for every neuron in every frame produces, for each neuron, a trajectory of embeddings over time. I
then compare embeddings by cosine similarity in two ways: \emph{within-neuron}, between the same
neuron in different frames, and \emph{between-neuron}, between different neurons in the same frame.
The gap, within minus between, measures how much more consistent a neuron is with itself over time
than with its neighbours. No labels enter the similarity itself; the footprints only say which
locations belong to which neuron.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/h3-embedding.png}
\caption{How a per-neuron embedding is formed in H3. \emph{Left:} the encoder turns a frame into a
dense feature map; the feature vectors at the locations inside a neuron's footprint are averaged into
a single embedding for that neuron in that frame. \emph{Right:} embeddings are compared by cosine
similarity, within-neuron (same neuron across frames) against between-neuron (different neurons in the
same frame). Own figure.}
\label{fig:h3emb}
\end{figure}
\label{sec:protocol}
