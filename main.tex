...
will let the approach outperform the baselines. The full code, checkpoints, and experiment log
...
\begin{itemize}
  \item \textbf{A reusable pipeline, not a script.} NEUROSEG is a single, pip-installable,
    ...
    is configured from a single file with no hardcoded paths (Section~\ref{sec:impl}).
...
\subsection{Implementation and Experiment Log}
\label{sec:impl}

The code is a pip-installable Python package (\texttt{neuroseg}) with a single CLI entry point; all
paths and hyperparameters come from CLI flags or a YAML config. A one-command demo (\texttt{demo.sh})
runs the full H1, H2, and H3 pipelines end to end on CPU on a real Neurofinder subset in about twenty
minutes, writing checkpoints, figures, and the experiment log. Training, inference, models, and data
loading are separated into distinct modules; checkpoints carry a JSON sidecar recording the exact
architecture and metrics; and smoke tests run in continuous integration on every push. The repository
(with a supervisor invited for grading) and the complete experiment log accompany this report;
Appendix~\ref{app:log} describes the log schema and hyperparameters.
