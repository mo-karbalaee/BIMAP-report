...
baseline at the field level (\pending{Cellpose Dice}), but that comparison
...
pretraining transfers to mouse with no meaningful penalty.}
\label{fig:h2}
\end{figure}
...
\paragraph{Values to finalize.} A few exact numbers, the field-level detection F1, the Cellpose
scores, and the exact test $p$-values, are still to be completed from a final full run and are marked
\pending{like this}. The qualitative conclusions already hold from the recorded results: H1
significant but $\Delta\approx0.02$ with a sign flip, H2 no significant cross-species penalty, H3
Cohen's $d=0.66$ versus $1.45$, and detection F1 $\approx 0.2$.
